...
# Classification Report (per document type)
class_report = classification_report(df["ground_truth_label"], df["MUT_prediction_label"], output_dict=True)
...
labels = sorted(set(df["ground_truth_label"].tolist() + df["MUT_prediction_label"].tolist()))
...
# Generate dynamic actual confusion matrix text
actual_conf_matrix_text = "             Predicted\n"
actual_conf_matrix_text += "             " + "-" * 21 + "\n"
header = "             " + "  ".join([label.capitalize() for label in labels])
actual_conf_matrix_text += header + "\n"
for i, label in enumerate(labels):
    row = "Actual | " + label.capitalize().ljust(8)
    for j in range(len(labels)):
        row += f"{conf_matrix[i, j]:>8}"
    actual_conf_matrix_text += row + "\n"

# Generate dynamic actual confidence score chart text (using defined bins)
...
# Static ideal texts (copied exactly from your text)
...
pdf.cell(0, 10, "AI-Assisted Evaluation Report", ln=1, align="C")
pdf.ln(5)

# Section 1: Overall Accuracy
pdf.set_font("Arial", 'B', 14)
pdf.cell(0, 10, "1. Overall Accuracy", ln=1)
pdf.set_font("Arial", '', 12)
pdf.multi_cell(0, 10, f"MUT correctly classified {correct_count} out of {total_count} documents.\nOverall Accuracy: {accuracy:.2%}")
pdf.ln(5)

# Section 2: Precision, Recall, and F1-Score
pdf.set_font("Arial", 'B', 14)
pdf.cell(0, 10, "2. Precision, Recall, and F1-Score (Per Document Type)", ln=1)
pdf.set_font("Arial", '', 12)
best_f1 = f"{doc_types[best_doc_type]['f1-score']:.2f}" if best_doc_type else "N/A"
worst_f1 = f"{doc_types[worst_doc_type]['f1-score']:.2f}" if worst_doc_type else "N/A"
pdf.multi_cell(0, 10, 
    f"Best Performance: {best_doc_type.capitalize() if best_doc_type else 'N/A'} (F1-score: {best_f1})\n"
    f"Worst Performance: {worst_doc_type.capitalize() if worst_doc_type else 'N/A'} (F1-score: {worst_f1})\n"
    "Detailed Metrics per Document Type:"
)
for doc, metrics in doc_types.items():
    pdf.cell(0, 10, f"{doc.capitalize()}: Precision = {metrics['precision']:.2f}, Recall = {metrics['recall']:.2f}, F1-Score = {metrics['f1-score']:.2f}, Support = {metrics['support']}", ln=1)
pdf.ln(5)

# Section 3: Misclassification Insights
pdf.set_font("Arial", 'B', 14)
pdf.cell(0, 10, "3. Misclassification Insights", ln=1)
pdf.set_font("Arial", '', 12)
for doc, stats in misclassification_insights.items():
...
# Section 4: Confidence Score Analysis
pdf.set_font("Arial", 'B', 14)
pdf.cell(0, 10, "4. Confidence Score Analysis", ln=1)
pdf.set_font("Arial", '', 12)
for doc, stats in confidence_stats.items():
    pdf.cell(0, 10, f"{doc.capitalize()}: Average MUT Confidence = {stats['avg_mut']:.2f}, Ground Truth Confidence = {stats['avg_gt']:.2f}, Drift = {stats['drift']:.2f}", ln=1)
pdf.ln(5)
pdf.multi_cell(0, 10, confidence_summary)
pdf.ln(10)

# Visual Insights
pdf.set_font("Arial", 'B', 14)
pdf.cell(0, 10, "Visual Insights", ln=1)
pdf.ln(5)
...
pdf.set_font("Arial", 'B', 14)
pdf.cell(0, 10, "Understanding the Charts: Ideal vs. Actual", ln=1)
pdf.ln(3)

# Confusion Matrix Subsection
pdf.set_font("Arial", 'B', 12)
pdf.cell(0, 10, "1. Understanding the Confusion Matrix", ln=1)
pdf.set_font("Arial", '', 11)
pdf.multi_cell(0, 8, "What is an 'Ideal' Confusion Matrix?\nAn ideal confusion matrix would have high values on the diagonal (correct classifications) and low values elsewhere (misclassifications).")
pdf.ln(2)
pdf.set_font("Arial", 'B', 11)
pdf.cell(0, 8, "Ideal Example (Perfect Classifier):", ln=1)
pdf.set_font("Courier", '', 10)
pdf.multi_cell(0, 8, ideal_conf_matrix_text)
pdf.ln(2)
pdf.set_font("Arial", '', 11)
pdf.multi_cell(0, 8, "Perfect Model - All correct classifications are along the diagonal, with no misclassifications.")
pdf.ln(2)
pdf.set_font("Arial", 'B', 11)
pdf.cell(0, 8, "What Does Our Actual Confusion Matrix Show?", ln=1)
pdf.set_font("Courier", '', 10)
pdf.multi_cell(0, 8, actual_conf_matrix_text)
pdf.ln(2)
pdf.set_font("Arial", '', 11)
pdf.multi_cell(0, 8, "Dynamic Insights: Compare the counts along the diagonal with the off-diagonal values to understand misclassifications.")
pdf.ln(5)

# Confidence Score Chart Subsection
pdf.set_font("Arial", 'B', 12)
pdf.cell(0, 10, "2. Understanding the Confidence Score Chart", ln=1)
pdf.set_font("Arial", '', 11)
pdf.multi_cell(0, 8, "What Would an 'Ideal' Confidence Score Chart Look Like?\nIdeally, Mistral and Azure should have very similar confidence scores for each document. The histogram bars should be concentrated around 0.0 - 0.2 (small differences).")
pdf.ln(2)
pdf.set_font("Arial", 'B', 11)
pdf.cell(0, 8, "Ideal Confidence Score Chart:", ln=1)
pdf.set_font("Courier", '', 10)
pdf.multi_cell(0, 8, ideal_conf_chart_text)
pdf.ln(2)
pdf.set_font("Arial", 'B', 11)
pdf.cell(0, 8, "What Does Our Actual Confidence Score Chart Show?", ln=1)
pdf.set_font("Courier", '', 10)
pdf.multi_cell(0, 8, actual_conf_chart_text)
pdf.ln(2)
pdf.set_font("Arial", '', 11)
pdf.multi_cell(0, 8, "Dynamic Insights: Larger bars toward higher differences indicate greater disagreement between models.")
pdf.ln(5)
